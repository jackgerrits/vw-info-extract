...
def find_first_dfs(
    node: clang.cindex.Cursor, pred: NodePredicate
...
def find_first_bfs(
    node: clang.cindex.Cursor, pred: NodePredicate, debug: bool = False
) -> Optional[clang.cindex.Cursor]:
    to_visit: Deque[Tuple[clang.cindex.Cursor, int]] = collections.deque([(node, 0)])
...
def handle_parse_args_file(node: clang.cindex.Cursor) -> List[str]:
    results: List[str] = []
    parse_reductions_decls = find_all(
        node, is_name_and_kind("parse_reductions", CursorKind.FUNCTION_DECL)
    )
    for parse_reductions_decl in parse_reductions_decls:
        push_backs = find_all(
            parse_reductions_decl, is_name_and_kind("push_back", CursorKind.CALL_EXPR)
        )
...
def handle_setup_fn(node: clang.cindex.Cursor) -> None:
    found = find_first_bfs(
        node, is_name_and_kind("set_prediction_type", CursorKind.CALL_EXPR)
    )
    if found:
    ...
    found = find_first_bfs(
        node, is_name_and_kind("set_label_type", CursorKind.CALL_EXPR)
    )
    if found:
        for arg in found.get_arguments():
            enum_type = arg.type.spelling
...
def handle_reduction_file(node: clang.cindex.Cursor, setup_fn_name: str) -> None:
    ...
    found = find_all(
        node,
        lambda node: setup_fn_name == node.spelling
        and node.kind == CursorKind.FUNCTION_DECL,
    )
    if len(found) != 0:
        assert len(found) >= 2
...
def generate_ast(
    file: str, index: clang.cindex.Index, includes: List[str]
) -> clang.cindex.Cursor:
...
if __name__ == "__main__":
    ...
    parse_setup_subparser.add_argument(
        "name",
        help="Setup function name to parse. Note doens't support namespaces yet.",
    )
